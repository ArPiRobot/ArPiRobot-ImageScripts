...
# Add script to regenerate ssh host keys
cat > /usr/local/last_boot_scripts/20-regenerate_ssh_keys_once.sh << 'EOF'
#!/usr/bin/env bash
rm /etc/ssh/ssh_host_*
ssh-keygen -A
rm /usr/local/last_boot_scripts/20-regenerate_ssh_keys_once.sh
EOF